(function() {
  ...
  argv = optimist.usage(['  usage: meh COMMAND', '    server  start a dynamic development server', '    build   serialize application to disk', '    watch   build & watch disk for changes'].join("\n")).alias('p', 'port').alias('d', 'debug').argv;
  ...
  Hem = (function() {
    ...
    Hem.prototype.server = function() {
      ...
      this.app.get(this.options.jsPath, this.hemPackage().createServerDev('app'));
      this.app.get(this.options.libsPath, this.hemPackage().createServerDev('lib'));
      this.app.get(this.options.dependencyPath, this.hemPackage().createServerDev('deps'));
    ...
    Hem.prototype.build = function() {
      ...
      source = this.hemPackage().compileApp(true);
      fs.writeFileSync(path.join(this.options.public, this.options.jsPath), source);
      source = this.hemPackage().compileLibrary(true);
      fs.writeFileSync(path.join(this.options.public, this.options.libsPath), source);
      source = this.hemPackage().compileDependencies(true);
      fs.writeFileSync(path.join(this.options.public, this.options.dependencyPath), source);
      console.log("compiled:");
      console.log(this.options.jsPath);
      console.log(this.options.libsPath);
      return console.log(this.options.dependencyPath);
    };
